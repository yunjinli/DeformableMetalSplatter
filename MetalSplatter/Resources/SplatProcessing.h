#ifndef SPLAT_PROCESSING_H
#define SPLAT_PROCESSING_H

#import "ShaderCommon.h"

static float3 calcCovariance2D(float3 viewPos, packed_half3 cov3Da,
                               packed_half3 cov3Db, float4x4 viewMatrix,
                               float4x4 projectionMatrix, uint2 screenSize) {
  ...
  float3x3 J = float3x3(focalX * invViewPosZ, 0, 0, 0, focalY * invViewPosZ, 0,
                        -(focalX * viewPos.x) * invViewPosZSquared,
                        -(focalY * viewPos.y) * invViewPosZSquared, 0);
  float3x3 W =
      float3x3(viewMatrix[0].xyz, viewMatrix[1].xyz, viewMatrix[2].xyz);
  float3x3 T = J * W;
  float3x3 Vrk = float3x3(cov3Da.x, cov3Da.y, cov3Da.z, cov3Da.y, cov3Db.x,
                          cov3Db.y, cov3Da.z, cov3Db.y, cov3Db.z);
...
// cov2D = ( a, b, d )
static void decomposeCovariance(float3 cov2D, thread float2 &v1,
                                thread float2 &v2) {
  float a = cov2D.x;
  float b = cov2D.y;
  float d = cov2D.z;
...
static bool isClusterSelected(uint clusterID, constant uint *selectedClusters,
                              uint count) {
...
static FragmentIn splatVertex(Splat splat, Uniforms uniforms,
                              uint relativeVertexIndex, uint splatIndex,
                              constant packed_float3 *clusterColors,
                              constant uint *clusterIDs,
                              constant uint *selectedClusters,
                              constant float *deformMasks) {
  FragmentIn out;

  uint thisClusterID =
      (clusterIDs != nullptr) ? clusterIDs[splatIndex] : 0xFFFFFFFF;
  bool clusterIsSelected = false;

  // Check if this cluster is in the multi-selection list
  if (uniforms.selectionMode > 0 && selectedClusters != nullptr &&
      uniforms.selectedClusterCount > 0) {
    clusterIsSelected = isClusterSelected(thisClusterID, selectedClusters,
                                          uniforms.selectedClusterCount);
  }
  ...
  // Filter by single selected cluster (legacy behavior) - only when not in
  // multi-selection mode
  if (uniforms.selectionMode == 0 && uniforms.selectedClusterID >= 0 &&
      clusterIDs != nullptr) {
  ...
  float3 cov2D = calcCovariance2D(
      viewPosition3, splat.covA, splat.covB, uniforms.viewMatrix,
      uniforms.projectionMatrix, uniforms.screenSize);
  ...
  half2 projectedScreenDelta = (relativeCoordinates.x * half2(axis1) +
                                relativeCoordinates.y * half2(axis2)) *
                               2 * kBoundsRadius / screenSizeFloat;

  out.position =
      float4(projectedCenter.x + projectedScreenDelta.x * projectedCenter.w,
  ...
  if (uniforms.showDepthVisualization != 0) {
    ...
    float t = saturate((depth - uniforms.depthRange.x) /
                       (uniforms.depthRange.y - uniforms.depthRange.x));
...
static half splatFragmentAlpha(half2 relativePosition, half splatAlpha) {
  half negativeMagnitudeSquared = -dot(relativePosition, relativePosition);
  return (negativeMagnitudeSquared < -kBoundsRadiusSquared)
             ? 0
             : exp(0.5 * negativeMagnitudeSquared) * splatAlpha;
}

#endif // SPLAT_PROCESSING_H
